(function($) {
	...
	$('.gla_animated_flower').each(function(){
  ...
  /*OWL Team*/
	...
	/* Submenu */
 	$('.gla_parent, .gla_default_menu .menu-item-has-children, .gla_default_menu .page_item_has_children').on({
		mouseenter:function(){
			$(this).find('> ul, .mega-menu').addClass('active');
		},mouseleave:function(){
			$(this).find('> ul, .mega-menu').removeClass('active');
		}
	});
	...
	$('.gla_music_icon').on('click',function(){
 	...
 	/* Mobile Menu */
	$('.gla_main_menu_content_menu .gla_parent, .gla_main_menu_content_menu .menu-item-has-children > a, .gla_main_menu_content_menu .page_item_has_children > a').on("click", function(e){
		$(this).next('.sub-menu').slideToggle(300);
		$(this).next('.mega-menu').find('ul').slideToggle(300);
	...
	$('.gla_default_menu a[href*=\\#]:not([href=\\#])').on("click", function(){
		$('.gla_default_menu a[href*=\\#]:not([href=\\#])').removeClass('active');
		$(this).addClass('active');
		if (location.pathname.replace(/^\//,'') == this.pathname.replace(/^\//,'') 
	        || location.hostname == this.hostname) {
	...
	$('.gla_middle_titles_effect h2').each(function(){
		$(this).attr('data-animation','animation_blocks').attr('data-bottom','@class:noactive').attr('data--100-bottom','@class:active');
	});

	/* Accordion */
	$('.gla_panel_heading').on("click", function(e){
		$(this).next('.gla_panel_body').slideToggle(300);
		$(this).toggleClass('active');
	});

	/* TABS */
	$('.gla_tab_group').each(function(){
		$(this).find('.gla_tab_heading .gla_tab_title:first-child').addClass('active');
	});
	$('.gla_tab_heading .gla_tab_title').on("click", function(e){
		var href = $(this).attr('data-href');
		$(this).next('.gla_panel_body').slideToggle(300);
		$(this).parents('.gla_tab_group').find('.gla_tab_body').hide();
		$(this).parents('.gla_tab_group').find('.gla_tab_title').removeClass('active');
		$(this).parents('.gla_tab_heading').next('.gla_tab').find('.gla_tab_body[data-id="'+href+'"]').fadeIn();
		$(this).addClass('active');
	});
	    ...
	    $("body").imagesLoaded(function(){
			...
			if( !device.tablet() && !device.mobile() ) {
				$('.sca_auto_height').each(function(){
					setEqualHeight($(this).find('.container-fluid  > div[class^="fw-col"]'));
				});
			}
